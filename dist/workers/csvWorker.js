...
function validateRow(row) {
    ...
    if (!name || !farmerCode)
        return { valid: false, error: 'name and farmer_code required' };
    return { valid: true, data: row };
}
async function processBatch(rows, tenantId, agentId) {
    ...
    for (const row of rows) {
        ...
        const farmerCode = String(data.farmer_code || data.farmerCode || data['Farmer Code']).trim();
...
async function streamCsvRows(s3Key) {
...
const worker = new Worker('csv-ingestion', async (job) => {
    const { jobId, s3Key, tenantId, agentId } = job.data;
    try {
...
worker.on('failed', (job, err) => logger.error('CSV job failed', { jobId: job?.id, error: err?.message }));
export { worker };
