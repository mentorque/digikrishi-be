    ...
    return new Promise((resolve, reject) => {
        conn.all(sql, ...params, (err, result) => {
            if (err)
                ...
                resolve(sanitizeForJson(result));
        });
    });
}
async function performSync(tenantId) {
    ...
    await runQuery(conn, `CREATE OR REPLACE TABLE farmers (
...
export async function getSummary(tenantId) {
    ...
    const kycResult = await runQuery(conn, "SELECT COUNT(*) as c FROM farmer_profile_details p JOIN farmers f ON p.farmer_id = f.id WHERE p.social_category IS NOT NULL AND p.social_category != ''");
    const kycCount = Number(kycResult[0]?.c ?? 0);
    const withPicResult = await runQuery(conn, "SELECT COUNT(*) as c FROM farmers WHERE profile_pic_url IS NOT NULL AND profile_pic_url != ''");
...
export async function getByDistrict(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(a.district, '—') as district, CAST(COUNT(f.id) AS INTEGER) as count
...
export async function getByState(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(a.state, '—') as state, CAST(COUNT(f.id) AS INTEGER) as count
     FROM farmers f LEFT JOIN farmer_addresses a ON f.id = a.farmer_id
...
export async function getByAgent(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT agent_id, CAST(COUNT(*) AS INTEGER) as count FROM farmer_agent_map
     GROUP BY agent_id ORDER BY count DESC`);
    return result;
}
export async function getBySocialCategory(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(p.social_category, '—') as social_category, CAST(COUNT(*) AS INTEGER) as count
     FROM farmer_profile_details p JOIN farmers f ON p.farmer_id = f.id
     GROUP BY p.social_category ORDER BY count DESC`);
    return result;
}
export async function getByGender(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(gender, '—') as gender, CAST(COUNT(*) AS INTEGER) as count
     FROM farmers GROUP BY gender ORDER BY count DESC`);
    return result;
}
export async function getByEducation(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(education, '—') as education, CAST(COUNT(*) AS INTEGER) as count
     FROM farmers GROUP BY education ORDER BY count DESC`);
    return result;
}
export async function getByKycStatus(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(kyc_status, 'PENDING') as kyc_status, CAST(COUNT(*) AS INTEGER) as count
     FROM farmers GROUP BY kyc_status ORDER BY count DESC`);
    return result;
}
export async function getByCaste(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(p.caste, '—') as caste, CAST(COUNT(*) AS INTEGER) as count
     FROM farmer_profile_details p JOIN farmers f ON p.farmer_id = f.id
     GROUP BY p.caste ORDER BY count DESC`);
    return result;
}
export async function getByFpc(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(p.fpc, '—') as fpc, CAST(COUNT(*) AS INTEGER) as count
     FROM farmer_profile_details p JOIN farmers f ON p.farmer_id = f.id
     GROUP BY p.fpc ORDER BY count DESC`);
    return result;
}
export async function getRationCardStats(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT
...
export async function getByVillage(tenantId, limit = 15) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(a.village, '—') as village, CAST(COUNT(f.id) AS INTEGER) as count
     FROM farmers f LEFT JOIN farmer_addresses a ON f.id = a.farmer_id
     GROUP BY a.village ORDER BY count DESC LIMIT ?`, [limit]);
    return result;
}
export async function getByMonth(tenantId) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT
...
export async function getByTaluka(tenantId, limit = 15) {
    const conn = await syncToDuckDb(tenantId);
    const result = await runQuery(conn, `SELECT COALESCE(a.taluka, '—') as taluka, CAST(COUNT(f.id) AS INTEGER) as count
     FROM farmers f LEFT JOIN farmer_addresses a ON f.id = a.farmer_id
     GROUP BY a.taluka ORDER BY count DESC LIMIT ?`, [limit]);
    return result;
}
